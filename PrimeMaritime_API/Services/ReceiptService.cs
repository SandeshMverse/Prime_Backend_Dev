...
namespace PrimeMaritime_API.Services
{
    public class ReceiptService : IReceiptService
    {
        ...
        public Response<List<RECEIPT_INVOICE>> GetReceiptList(string FROM_DATE, string TO_DATE, string PORT, string ORG_CODE)
        {
            string dbConn = _config.GetConnectionString("ConnectionString");

            Response<List<RECEIPT_INVOICE>> response = new Response<List<RECEIPT_INVOICE>>();
            var data = DbClientFactory<ReceiptRepo>.Instance.GetReceiptList(dbConn, FROM_DATE, TO_DATE, PORT, ORG_CODE);
